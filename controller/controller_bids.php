...
if (!isset($_GET["id_bid"]) && !isset($_GET["bid"])){
  ...
  $minute = date("i");
  $second = date("s");
  $req_user =" SELECT * FROM articles WHERE status='bid'";//1
  $data = $db_connexion->query($req_user)->fetchAll();
  echo "<center><h2>Bids</h2></center><div id='box'><br>";
  ...
  if (count($data) > 0 ){
    for ($i=0; $i < count($data) ; $i++) {
      ...
      if ( $date_end[0] > $year ||$date_end[0] == $year && $date_end[1] > $month ||$date_end[0] == $year && $date_end[1] == $month && $date_end[2] > $day ||$date_end[0] == $year && $date_end[1] == $month && $date_end[2] == $day && $hour_end[0] > $hour ||$date_end[0] == $year && $date_end[1] == $month && $date_end[2] == $day && $hour_end[0] == $hour && $hour_end[1] > $minute ) {
        ...
        if ( empty($bidder[1])){
          $bid_der = "No bidder for this offer .";
        } else {
          $bid_der = $bidder[1];
        }

        echo "Last bidder :".$bid_der."<br>";
    ...
    echo "<br>There is no bid online .";
  }
    echo "<br><br><br></div>";
