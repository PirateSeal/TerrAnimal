<h2><center>Your Commands</h2></center>
  <div id="box">
  <?php
      ...
      if (!empty($transactions)) {
        if (!isset($_POST['ID'])) {
          for ($i=0; $i<count($transactions) ; $i++) {
              echo '<table style="font-size: 20px"  class="table" border="1" cellpadding="5" cellspacing="0.5" width="100%">
                      <tr align = "center">
          ...
          for ($i=0; $i<count($transactions) ; $i++) {
            if ($transactions[$i]['id_transaction_line'] == $_POST['tl']) {
              echo '<table style="font-size: 20px" border = 1  class="table" cellpadding="5" cellspacing="0.5" width="100%">
                      <tr align = "center">
              ...
              if(isset($transactions[$i]['vote_token'])) {
                if ($transactions[$i]['vote_token'] == 1) {
