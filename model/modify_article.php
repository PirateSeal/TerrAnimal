<?php
		...
		$sql = 'update articles set description="'.$_GET["description"].'", id_specie="'.$specie.'" unit_price="'.$_GET["unit_price"].'", stock="'.$_GET["stock"].'", gender="'.$_GET["gender"].'", diet="'.$_GET["diet"].'", weight="'.$_GET["weight"].'", size="'.$_GET["size"].'", color="'.$_GET["color"].'", age="'.$_GET["age"].'"  WHERE id_user="'.$id.'"';
		$pdo->exec($sql);
